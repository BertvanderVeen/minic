...
#' @param regularize logical. Defaults to TRUE. Should regularization be implemented?
#' @param quasi logical. Defaults to TRUE. If FALSE implements regularised Newton optimization.
#' @param method The (limited-memory) method to be used when \code{quasi = TRUE}. Defaults to "BFGS", alternatives are "SR1" and "PSB".
...
#' All quasi-Newton methods implemented are of the limited-memory kind. The function start with a single More-Thuente line search along the normalized negative gradient direction. 
...
maxit = control$maxit;m=control$m;sigma1=control$sigma1;sigma2=control$sigma2;c1=control$c1;pmin=control$pmin;c2=control$c2;tol.g=control$tol.g;tol.gamma=control$tol.gamma;tol.obj=control$tol.obj;tol.mu=control$tol.mu;tol.mu2=control$tol.mu2;tol.c=control$tol.c;report.iter=control$report.iter
...
if(sigma2<1)stop("sigma2 must be larger than 1.")

method = switch(method, "BFGS" = 1, "SR1" = 2, "PSB" = 3)
